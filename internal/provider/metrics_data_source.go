...
func (d *metricsDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	...
	resp.Schema = schema.Schema{MarkdownDescription: "Retrieves all Coval metrics matching public API filters.", Attributes: map[string]schema.Attribute{
		...
		"metrics": schema.ListNestedAttribute{MarkdownDescription: "All matching metric summaries. Use the singular coval_metric data source for polymorphic and nested configuration.", Computed: true, NestedObject: schema.NestedAttributeObject{Attributes: map[string]schema.Attribute{
			...
			"direction": cS(), "success_sentiments": cSet(), "percent_above": cF(), "success_end_reasons": cSet(), "observation_name": cS(), "match_path": cS(), "min_volume_change_for_pitch_misalignment": cF(), "threshold": schema.Int64Attribute{Computed: true}, "operator": cS(), "sql_query": cS(), "aggregation_method": cS(), "unit": cS(),
...
func (d *metricsDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	for index, remote := range all {
		...
		resp.Diagnostics.Append(diagnostics...)
		config.Metrics[index] = metricSummaryModel{Name: types.StringValue(remote.Name), ID: types.StringValue(remote.ID), MetricName: types.StringValue(remote.MetricName), Description: types.StringValue(remote.Description), MetricType: types.StringValue(remote.MetricType), Prompt: nullableString(remote.Prompt), EnabledTools: set(remote.EnabledTools), Categories: set(remote.Categories), MinValue: nullableFloat(remote.MinValue), MaxValue: nullableFloat(remote.MaxValue), MetadataFieldType: nullableString(remote.MetadataFieldType), MetadataFieldKey: nullableString(remote.MetadataFieldKey), RegexPattern: nullableString(remote.RegexPattern), Role: nullableString(remote.Role), MinPauseDurationSeconds: nullableFloat(remote.MinPauseDurationSeconds), MaxSilenceDurationSeconds: nullableFloat(remote.MaxSilenceDurationSeconds), MinSilenceGapSeconds: nullableFloat(remote.MinSilenceGapSeconds), FrequencyThreshold: nullableFloat(remote.FrequencyThreshold), Direction: nullableString(remote.Direction), SuccessSentiments: set(remote.SuccessSentiments), PercentAbove: nullableFloat(remote.PercentAbove), SuccessEndReasons: set(remote.SuccessEndReasons), ObservationName: nullableString(remote.ObservationName), MatchPath: nullableString(remote.MatchPath), MinVolumeChangeForPitchMisalignment: nullableFloat(remote.MinVolumeChangeForPitchMisalignment), Threshold: nullableInt(remote.Threshold), Operator: nullableString(remote.Operator), SQLQuery: nullableString(remote.SQLQuery), AggregationMethod: nullableString(remote.AggregationMethod), Unit: nullableString(remote.Unit), CriteriaSource: nullableString(remote.CriteriaSource), CriteriaPath: nullableString(remote.CriteriaPath), Criteria: set(remote.Criteria), ReportingMethod: nullableString(remote.ReportingMethod), BasePromptTemplate: nullableString(remote.BasePromptTemplate), IncludeTraces: nullableBool(remote.IncludeTraces), Tags: tags, CreatedBy: nullableString(remote.CreatedBy), CreateTime: types.StringValue(remote.CreateTime), UpdateTime: nullableString(remote.UpdateTime)}
	}
	if resp.Diagnostics.HasError() {
		return
	}
